...
export function OrderModal({
  ...
  const fieldClass = (key: string) =>
    `${inputClass} ${fieldErrors[key] ? "border-red-500/80 focus:border-red-500" : ""}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setName((current) => current || savedAddress?.recipient_name || profile?.display_name || "");
      setPhone((current) => current || savedAddress?.phone || profile?.phone || "");
  ...
  const useCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setLatitude(lat);
        setLongitude(lng);
        setMapUrl(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${lat},${lng}`)}`);
        setLocating(false);
      },
      () => {
        setLocating(false);
  ...
  const applyPromo = async () => {
    ...
    setPromoBusy(true);
    setError("");
    const { data, error: promoError } = await (supabase as any).rpc("preview_promo_code", {
  ...
  const submit = async () => {
    ...
    if (address.trim().length < 5) nextErrors.address = "Enter your full delivery address.";
          ...
          {orderNumber ? (
            <div className="py-8 text-center">
              ...
              <span className="mt-7 block text-[9px] uppercase tracking-[0.45em] text-muted-foreground">
                ZZERKOFF / {preorder ? "PRE-ORDER" : "ORDER"}
              </span>
              ...
              <div className="mx-auto mt-6 max-w-sm rounded-[20px] border border-border/60 bg-white/[0.02] p-4 text-left">
                ...
                <div className="mt-4 flex items-end justify-between gap-4 border-t border-border/40 pt-4">
                  ...
                  <p className="text-xs tracking-[0.14em] text-foreground">
                    {currencySymbol}{Number(placedTotal ?? total).toLocaleString("en-US")}
                  </p>
                </div>
              </div>

              <p className="mx-auto mt-6 max-w-sm text-xs leading-relaxed tracking-[0.08em] text-muted-foreground">
                Keep your order ID. You can track it anytime from Track Order.
              </p>
            ...
            <>
              <span className="text-[9px] uppercase tracking-[0.45em] text-muted-foreground">
                ZZERKOFF / {preorder ? "PRE-ORDER" : "CHECKOUT"}
              </span>
              <h2 className="mt-4 pr-12 font-display text-xl tracking-[0.2em] text-foreground">{title}</h2>

              <div className="mt-6 rounded-[20px] border border-border/60 bg-white/[0.02] p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <span className="text-[8px] uppercase tracking-[0.35em] text-muted-foreground">{productCode}</span>
                    <p className="mt-2 font-display text-sm tracking-[0.14em] text-foreground">{productName}</p>
              ...
              <div className="mt-5 rounded-[20px] border border-border/60 bg-white/[0.015] p-4">
                <label className="text-[8px] uppercase tracking-[0.32em] text-muted-foreground">Promo / coupon</label>
                ...
                {promo?.valid ? (
                  <p className="mt-3 text-[9px] text-chrome">
                    {promo.code}: −{currencySymbol}{promo.discount_amount.toLocaleString("en-US")}
                  </p>
                ) : null}
              </div>
              ...
              <div className="mt-6 space-y-4">
                <div>
                  <label className="mb-2 block text-[8px] uppercase tracking-[0.35em] text-muted-foreground">Name *</label>
                  <input
                    value={name}
                    onChange={(event) => {
                ...
                <div>
                  <label className="mb-2 block text-[8px] uppercase tracking-[0.35em] text-muted-foreground">Email</label>
                  <input
                    type="email"
                    value={email}
                    onChange={(event) => {
                ...
                <div>
                  <label className="mb-2 block text-[8px] uppercase tracking-[0.35em] text-muted-foreground">Full delivery address *</label>
                  <textarea
                    rows={3}
                    value={address}
                    onChange={(event) => {
                ...
                >
                  {locating ? <Loader2 className="size-4 animate-spin" /> : mapUrl ? <Check className="size-4" /> : <MapPin className="size-4" />}
                ...
                {mapUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center gap-2 text-[8px] uppercase tracking-[0.3em] text-chrome transition-colors hover:text-foreground"
                  >
                    View map pin
                ...
                <div>
                  <label className="mb-2 block text-[8px] uppercase tracking-[0.35em] text-muted-foreground">Payment method</label>
                  <div className="space-y-2">
                    {paymentSettings?.cod_enabled !== false && (
                      <button
                        ...
                        className={`w-full rounded-2xl border px-4 py-4 text-left ${paymentMethod === "cod" ? "border-chrome/60 bg-white/[0.05]" : "border-border/60 bg-white/[0.02]"}`}
                      >
                    ...
                    {paymentSettings?.bkash_enabled && paymentSettings?.bkash_number && (
                      <button
                        ...
                        className={`w-full rounded-2xl border px-4 py-4 text-left ${paymentMethod === "bkash" ? "border-chrome/60 bg-white/[0.05]" : "border-border/60 bg-white/[0.02]"}`}
                      >
                    ...
                    {paymentSettings?.nagad_enabled && paymentSettings?.nagad_number && (
                      <button
                        ...
                        className={`w-full rounded-2xl border px-4 py-4 text-left ${paymentMethod === "nagad" ? "border-chrome/60 bg-white/[0.05]" : "border-border/60 bg-white/[0.02]"}`}
                      >
                  ...
                  {paymentMethod !== "cod" && (
                    <div className="mt-3">
                      <label className="mb-2 block text-[8px] uppercase tracking-[0.35em] text-muted-foreground">Transaction ID *</label>
                      <input
                        value={transactionId}
                        onChange={(event) => {
                ...
                <div>
                  <label className="mb-2 block text-[8px] uppercase tracking-[0.35em] text-muted-foreground">Order note</label>
                  <textarea
                    rows={3}
                    value={note}
              ...
              >
                {submitting ? <Loader2 className="size-4 animate-spin" /> : <ArrowUpRight className="size-4" />}
